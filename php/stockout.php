...
<?php
include '../connect.php';
$select = $conn->query("select*,sum(totalprice),sum(quantity) from stockout ,product where product.id=stockout.pid 
group by id,date");
$selecttot = $conn->query("select sum(totalprice) from stockout");
...
if (isset($_POST['gen'])){
    if($_POST['gen'] == "genereteday"){
        $select = $conn->query("select*,sum(totalprice),sum(quantity) from stockout ,product where product.id=stockout.pid 
         and date='{$_POST['date']}' group by id,date order by date desc");
        $selecttot = $conn->query("select sum(totalprice) from stockout where date='{$_POST['date']}'");
        $tot = mysqli_fetch_array($selecttot);
    }
    if($_POST['gen'] == "generateweek"){
        $select = $conn->query("select*,sum(totalprice),sum(quantity) from stockout ,product where product.id=stockout.pid 
        and date between '{$_POST['sdate']}' and '{$_POST['edate']}' group by id,date order by date desc");
        $selecttot = $conn->query("select sum(totalprice) from stockout where date between '{$_POST['sdate']}' and '{$_POST['edate']}'");
        $tot = mysqli_fetch_array($selecttot);
    }
}
?>
